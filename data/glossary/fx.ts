...
export const fxTerms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    explanation:
      "各平台的货币转换在三个维度上差异很大：一是收费方式（百分比加点、固定最低费或额度内免费）；二是是否形成可自由支配的目标货币余额；三是是否受额度与时间限制。Revolut Standard 在工作日、当月换汇额度内不收明确手续费，周末则收 1%，超出公平使用额度也可能收费。Robinhood Europe 的换汇只形成 Token 购买力，不产生可提取的美元现金；Trade Republic 则完全没有独立的美元余额，股票与 ETF 以欧元报价结算。",
    whyExists:
